...
**Status as of 2026-09-16 (end of the post-M8 stabilization pass):** the app is deployed at
...
| # | What | Gates | Cost |
|---|---|---|---|
| 1 | **Send the M8 commits to GitHub** (seven, plus the account-row fix once you approve it). Everything is committed and the working tree is clean; the pipeline is green. Vercel redeploys on receipt, so **the live site shows the pre-M8 UI until you do this.** | The whole redesign reaching the live site | 1 min |
...
| 8 | **Date inputs show US `mm/dd/yyyy`** because `<html lang="en">` resolves to en-US. Left alone on purpose — making it `dd/mm/yyyy` means going locale-aware across every page and seven pre-existing inputs, which is not worth doing right now. Say the word if you disagree. | Nothing | — |
...
- ~~The account name colliding with its balance at phone width~~: fixed in the post-M8
  stabilization pass, with a regression test that measures the gap rather than trusting the eye.
