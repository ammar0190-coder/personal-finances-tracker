...
async function recompute(client: ReturnType<typeof createAdminClient<Database>>, entryId: string) {
  const { data: entry } = await client.from("iou_entries").select("*").eq("id", entryId).single();
  const { data: linked } = await client.from("transactions").select("*").eq("related_iou_entry_id", entryId);
...
describe.runIf(process.env.RUN_RLS_TESTS === "1")("IOU & Reimbursements (PRD §7), live", () => {
  ...
  beforeAll(async () => {
    ...
    bankId = bank!.id;
    const { data: category } = await client.from("categories").insert({ user_id: userId, name: "Leisure", kind: "expense" }).select().single();
